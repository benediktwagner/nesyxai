...
class Not_Godel:
    def __call__(self,x):
        return torch.equal(x,torch.tensor(0., device=x.device)).type(x.dtype)
...
class And_Luk:
    def __call__(self,x,y):
        return torch.max(x+y-1,torch.tensor(0., device=x.device))
...
class Or_Luk:
    def __call__(self,x,y):
        return torch.min(x+y,torch.tensor(1., device=x.device))
...
class Implies_Luk:
    def __call__(self,x,y):
        return torch.min(1.-x+y,torch.tensor(1., device=x.device))
