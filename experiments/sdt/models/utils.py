...
def draw_tree(sess, tree, img_rows, img_cols,
    ...
    for pos, key in enumerate(sorted(kernels.keys(), key=lambda x:(len(x), x))):
        ...
        ax.imshow(kernel_image.astype(np.uint8), **imshow_args) # added .astype(np.uint8)
        ax.axis('off')
    ...
    for pos, key in enumerate(sorted(axes.keys(), key=lambda x:(len(x), x))):
        ...
        for child_key in children_keys:
            #p_rows, p_cols, chan = axes[key].get_images()[0].get_array().shape
